...
void setColorPalette (uint8_t color, uint8_t red, uint8_t green, uint8_t blue)
{
	...
	uint8_t reg_index = color >> 1; // color_reg[3:1];
	uint32_t val = hdmi_ctrl -> PALETTE[reg_index];

	if ((color & 1) == 0) { // LSB determines lower or upper half
		// lower half: keep upper half and modify lower half
		val = (val & 0xFFFF0000) | ((red & 0xF) << 8) | ((green & 0xF) << 4) | (blue & 0xF);
	} else {
		// upper half: keep lower half and modify upper half
		val = (val & 0x0000FFFF) | ((red & 0xF) << 24) | ((green & 0xF) << 20) | ((blue & 0xF) << 16);
	}
	hdmi_ctrl -> PALETTE[reg_index] = val;
}
